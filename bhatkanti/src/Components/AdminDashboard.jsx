...
const AdminDashboard = () => {
  ...
  const handleAddPlace = async () => {
    try {
      ...
      await axios.post('http://localhost:8080/admin/places/add', newPlace, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setOpenAddPlaceDialog(false);
      setNewPlace({ placeName: '', cityId: '', description: '' });
  ...
  const handleDeletePlace = async (placeId) => {
    try {
      ...
      await axios.delete(`http://localhost:8080/admin/places/${placeId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData(); // Refresh the place list
    } catch (error) {
                          ...
                          <TableBody>
                            {places.map((place) => (
                              <TableRow key={place.place_id}>
                                ...
                                <TableCell>{place.city_id}</TableCell>
                                <TableCell>{place.description}</TableCell>
                                <TableCell>
                                  <Button variant="contained" color="secondary" onClick={() => handleDeletePlace(place.place_id)}>
                                    Delete
                                  </Button>
                                </TableCell>
                              </TableRow>
